...
公开仓库默认不提交任何真实用户命例、姓名、原始分析记录或私密上下文。`manifest.json` 保持为空数组，维护者可在本地私有目录加入 fixture 做回归，但不要推送到 GitHub。

## 文件结构

- `*.txt`：原始文墨盘文本，尽量保持 App / API 导出的树状格式，不要改写成分析报告。
- `manifest.json`：每个样本的出生信息、基础预期字段和关键校验点。
