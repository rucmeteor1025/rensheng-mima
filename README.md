...
The public `xiashensuan_core/fixtures/synthetic/` contains only deterministic synthetic fixtures generated by `scripts/gen_synthetic_fixtures.py`; no real user records are included. Maintainers can keep private fixture records locally under `private_records/`, which is ignored by Git.
...
Public regression over synthetic fixtures is always run: regenerate them with
